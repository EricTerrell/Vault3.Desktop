	...
	public static boolean sort(List<OutlineItem> items) {
		...
		for (int i = 0; i < parent.getChildren().size(); i++) {
			for (int j = 0; j < parent.getChildren().size(); j++) {
				if (i != j) {
					OutlineItem item1 = parent.getChildren().get(i);
					OutlineItem item2 = parent.getChildren().get(j);
					
					if (itemsToSort.contains(item1) && itemsToSort.contains(item2)) {
						if (collator.compare(item1.getTitle(), item2.getTitle()) < 0)  {
							swap(parent, i, j, false);
							itemsSwapped = true;
						}
					}
				}
