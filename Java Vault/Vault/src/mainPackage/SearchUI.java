...
public class SearchUI extends Composite {
	...
	private ComboViewer searchComboViewer;
	
	private Search.SearchResults searchResults;
	
	private Button clearButton, allWordsRadioButton, atLeastOneWordRadioButton, matchWholeWordsOnlyCheckBox, searchAllRadioButton,
	               searchSelectedRadioButton, matchCaseCheckBox, searchTitlesRadioButton, searchTitlesAndTextRadioButton;
	...
	private void updateGuiForNewSearch() {
		StructuredSelection structuredSelection = (StructuredSelection) searchComboViewer.getSelection();
		
		if (!structuredSelection.isEmpty()) {
			SearchParameters searchParameters = (SearchParameters) structuredSelection.getFirstElement();
	...
	SearchUI(Composite parent) {
		...
		Composite searchForComposite = new Composite(composite, SWT.NONE);
		...
		Label searchForLabel = new Label(searchForComposite, SWT.NONE);
		...
		Label imageLabel = new Label(searchForComposite, SWT.NONE);
		imageLabel.setImage(image);
		
		imageLabel.setToolTipText(StringLiterals.SearchTextToolTip);
		
		Button clearSearchHistoryButton = new Button(searchForComposite, SWT.PUSH);
		...
		Group searchTargetGroup = new Group(composite, SWT.NONE);
		...
		Group matchGroup = new Group(composite, SWT.NONE);
		...
		Group multipleWordSearchOptionsGroup = new Group(composite, SWT.NONE);
		...
		Composite searchClearComposite = new Composite(composite, SWT.NONE);
		...
		Composite searchResultsComposite = new Composite(composite, SWT.NONE);
		gridData = new GridData(SWT.FILL);
		gridData.grabExcessHorizontalSpace = true;
		...
		searchButton.addSelectionListener(new SelectionAdapter() {
			...
			private void saveSearchParameters(SearchParameters searchParameters) {
				...
				int maxSavedSearches = Globals.getPreferenceStore().getInt(PreferenceKeys.MaxSavedSearches);
				...
				ArrayList<SearchParameters> searchParametersList = new ArrayList<>();
	...
	private static String getSavedSearchesKey() {
		return String.format("SavedSearches_%s", Globals.getVaultDocument().getFileName().trim().toLowerCase());
	}
	
	private void loadSavedSearches() {
		String serializedText = Globals.getPreferenceStore().getString(getSavedSearchesKey());
		
		List<SearchParameters> searchParametersList = SearchParameters.deserialize(serializedText);
